...
class AdminLaporanTransaksiController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		$this->col[] = ["label" => "Event", "name" => "event", "visible" => true];
		...
		$this->col[] = ["label" => "Harga", "name" => "harga_2", "callback" => function ($row) {
			// disable jika statusnya tidak pending
			$disabled = $row->status == "5" ? 'disabled' : '';
			$html_input = "<input type='text' name='harga_2' id='harga_" . $row->id . "' class='form-control text-right numberonly'  onchange='UbahHarga(" . $row->id . ",this.value)' value='" . number_format($row->harga_2) . "' " . $disabled . ">";
			return $html_input;
		}];
		// buat select untuk merubah status lunas / belum
		$this->col[] = ["label" => "Lunas", "name" => "lunas_2", "callback_php" => '$row->lunas_2 == "Y" ? "<span class=\"label label-success\">Lunas</span>" : "<span class=\"label label-danger\">Belum Lunas</span>"'];
		...
		$this->form = [];
		// layanan
		$this->form[] = ['label' => 'Layanan', 'name' => 'layanan', 'type' => 'text', 'validation' => '', 'width' => 'col-sm-10','readonly' => true];
		// event
		$this->form[] = ['label' => 'Event', 'name' => 'event', 'type' => 'text', 'validation' => '', 'width' => 'col-sm-10'];
		...
		$this->form[] = ['label' => 'Status Bayar', 'name' => 'lunas_2', 'type' => 'radio', 'validation' => '', 'width' => 'col-sm-10', 'dataenum' => 'N|Belum Lunas;Y|Lunas'];
		...
		$this->script_js = "
			function UbahHarga(id,harga){			
				$.get('/transaksis-all/ubah-harga/'+id+'/'+harga,function(data){
					// console.log(data);
			...
			function UbahLunas(id,status){
				$.get('/transaksis-all/ubah-status/'+id+'/'+status,function(data){
					// console.log(data);					
				});
			}
			";
		...
		$pembayaran = request()->get('pembayaran') ?? 'all';
		// $this->pre_index_html = '';
		$this->pre_index_html = '
		...
		$dataku = DB::table('transaksis')->where('layanan', 'rapat');
		$dlunas  = DB::table('transaksis')->where('layanan', 'rapat');
		$dbelum_lunas = DB::table('transaksis')->where('layanan', 'rapat');
		...
		if ($col_fil) {
			...
			$total_harga = $dataku->sum('harga_2');
			$datalunas = $dlunas->where('lunas_2', 'Y');
			$databelumlunas = $dbelum_lunas->where('lunas_2', '<>', 'Y');

			$lunas = $datalunas->count();
			$rp_lunas = $datalunas->sum('harga_2');
			$belum_lunas = $databelumlunas->count();
			$rp_belum_lunas = $databelumlunas->sum('harga_2');
		} else {
			$col_fil = request()->all();
			if ($col_fil['periode_awal'] || $col_fil['periode_akhir']) {
				$dataku->whereBetween('tanggal', [$col_fil['periode_awal'], $col_fil['periode_akhir']]);
			}

			if ($col_fil['status'] != 'all') {
				$dataku->where('status', $col_fil['status']);
			}
			if ($col_fil['layanan'] != 'all') {
				$dataku->where('layanan', $col_fil['layanan']);
			}
			if ($col_fil['cost_center'] != 'all') {
				$dataku->where('cost_center', $col_fil['cost_center']);
			}
			$total_data = $dataku->count();
			$total_harga = $dataku->sum('harga_2');

			$data_lunas_y = clone $dataku;
			$belum_lunas = $dataku->where('lunas_2', 'N')->count();
			$rp_belum_lunas = $dataku->sum('harga_2');

			$lunas = $data_lunas_y->where('lunas_2', 'Y')->count();
			$rp_lunas = $data_lunas_y->sum('harga_2');
		}
				...
				<div class="panel-heading">Data Transaksi</div>
					<div class="row">
						<div class="col-sm-12" style="margin-left: 20px;font-size: large;">
							...
							<span>Total Transaksi : ' . $total_data . ' Transaksi</span> <span>( Rp. ' . number_format($total_harga, 0, ',', '.') . ' )</span><br>
						</div>
					</div>					
				</div>
			</div>
	...
	public function hook_query_index(&$query)
	{
		...
		// filter cost_center jika ada
		if (request('pembayaran') && request('pembayaran') != 'all') {
			$query->where('transaksis.lunas', request('pembayaran'));
		}
	...
	public function UbahHarga($id, $harga)
	{
		...
		if ($transaksi_detail) {
			$transaksi_detail->harga_2 = $harga;
	...
	public function UbahStatus($id, $status)
	{
		$transaksi_detail = Transaksi::where('id', $id)->first();
		if ($transaksi_detail) {
			$transaksi_detail->lunas_2 = $status;
			$transaksi_detail->last_edit_id = CRUDBooster::myId();
			$transaksi_detail->save();
			return $status;
		}
	...
	public function laporan(Request $request)
	{
		...
		$query = [];
		return view('laporan.harian', compact('data', 'query', 'cost_centers'));
	}
