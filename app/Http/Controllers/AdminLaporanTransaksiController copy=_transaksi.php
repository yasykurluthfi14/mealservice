...
class AdminLaporanTransaksiController extends \crocodicstudio\crudbooster\controllers\CBController
{
	public function cbInit()
	{
		...
		if ($col_fil) {
			...
			$databelumlunas = $dbelum_lunas->where('status', '<=', 1)->whereRaw('harga <= 0');
	...
	public function hook_query_index(&$query)
	{
		//Your code here
		// filter_column
		// $query->where('<transa></transa>ksis.status', 1)->whereRaw('harga <= 0');

		// filter periode_awal & periode_akhir jika ada
		// dd(request('periode_akhir'), request('periode_awal'));
