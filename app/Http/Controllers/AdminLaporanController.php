...
class AdminLaporanController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		$this->col[] = ["label" => "Judul Rapat", "name" => "rapat_title"];
		$this->col[] = ["label" => "Ruang Rapat", "name" => "rapat_room", "join" => "ruang_meeting,ruang_rapat"];
		$this->col[] = ["label" => "Jam", "name" => "jam"];
		$this->col[] = ["label" => "S/D Jam", "name" => "jam_end"];
		...
		$this->col[] = ["label" => "Produk", "name" => "items"];
		...
		$this->form = [];
		$this->form[] = ['label' => 'Trxid', 'name' => 'trxid', 'type' => 'text', 'width' => 'col-sm-10', 'readonly' => '1'];
		$this->form[] = ['label' => 'Keterangan', 'name' => 'keterangan', 'type' => 'text', 'width' => 'col-sm-10', 'readonly' => '1'];
		$this->form[] = ['label' => 'Tanggal', 'name' => 'tanggal', 'type' => 'text', 'width' => 'col-sm-10', 'readonly' => '1'];
		$this->form[] = ['label' => 'Status Bayar', 'name' => 'lunas', 'type' => 'radio', 'width' => 'col-sm-10', 'dataenum' => 'N|Belum Lunas;Y|Lunas'];
		// $this->form[] = ['label'=>'Transaksi Detail','name'=>'transaksi_details','type'=>'child','width'=>'col-sm-10','table'=>'transaksi_details','foreign_key'=>'transaksi_id'];
		...
		if ($col_fil) {
			...
			$belum_lunas = $dataku->where('lunas', 'N')->count();
			$rp_belum_lunas = $dataku->sum('harga');

			$lunas = $data_lunas_y->where('lunas', 'Y')->count();
			$rp_lunas = $data_lunas_y->sum('harga');
		}
	...
	public function hook_query_index(&$query)
	{
		...
		if (request('periode_awal') && request('periode_awal') && request('periode_akhir')) {
